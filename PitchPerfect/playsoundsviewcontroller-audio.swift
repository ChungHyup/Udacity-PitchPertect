...
extension PlaySoundsViewController: AVAudioPlayerDelegate {
    ...
    func playSound(rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        self.durationLabel.text = String(format:"/%.2d:%.2d", Int(audioDuration/changeRatePitchNode.rate)/60, Int(audioDuration/changeRatePitchNode.rate)%60)
    ...
    func scheduleStopTimer(rate: Float? = nil, isRestart: Bool? = false){
        ...
        if isRestart! {
            ...
            let length = Double(self.audioFile.length - self.restartPoint!)/audioPlayerTime.sampleRate
            if let rate = rate {
                delayInSeconds = length/Double(rate)
            }else{
                delayInSeconds = length
            }
    ...
    func stopAudio() {
        ...
        if let currentTimeTimer = currentTimeTimer{
